...
def train(model, n_iters, learning_rate, cont=True):
    ...
    for epoch in range(n_iters):
        ...
        if (epoch + 1) % 100 == 0:
            ...
            }, './2Dadvection/data driven/model/checkpoint_fno.pt')
...
def load_model(model):
    ...
    checkpoint = torch.load('./2Dadvection/data driven/model/checkpoint_fno.pt')
...
if __name__ == '__main__':
    ...
    def get_grid(k, Nx, Ny):
        ...
        gridx = torch.tensor(np.linspace(-0.6, 0.6, size_x, endpoint=False), dtype=torch.float)
        gridx = gridx.reshape(1, size_x, 1, 1).repeat([batchsize, 1, size_y, 1])
        gridy = torch.tensor(np.linspace(-0.6, 0.6, size_y, endpoint=False), dtype=torch.float)
