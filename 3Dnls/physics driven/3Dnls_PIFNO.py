...
class RONN(nn.Module):
    ...
    def forward(self, x):
        ...
        for step in range(self.step - self.k):
            ...
            u = o[0, :, :, :, 0]
            v = o[0, :, :, :, 1]
            Lu = self.spectral_laplacian_3d(u)
            Lv = self.spectral_laplacian_3d(v)
    ...
    def spectral_laplacian_3d(self, u, Lx=6.0, Ly=6.0, Lz=6.0):
        """
...
def train(model, x, n_iters, learning_rate, cont=True):
    ...
    for epoch in range(n_iters):
        ...
        if (epoch + 1) % 100 == 0:
            ...
            }, './3Dnls/physics driven/model/checkpoint_pifno.pt')
...
def load_model(model):
    ...
    checkpoint = torch.load('./3Dnls/physics driven/model/checkpoint_pifno.pt')
...
def postprocess3D(data, x, y, z, flag=None, num=None, t=None):
    ...
    # fig.show()
    fig.write_image('./3Dnls/figures/True/Iso_surf_true_%s_%.2f.png' % (appd[flag], t[num]), engine='orca', scale=2)
    plt.close('all')
...
if __name__ == '__main__':
    ...
    if len(indices) > 0:
        ...
        print(f"第一个大于0.1的位置是索引 {first_index + 5}，对应值 {first_value}")
