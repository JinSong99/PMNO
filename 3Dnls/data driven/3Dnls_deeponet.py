...
def train(model, y, n_iters, learning_rate, cont=True):
    ...
    for epoch in range(n_iters):
        ...
        W = torch.exp(-1000 * (loss_t @ M)).detach()
        loss_w = 5000 * (W * loss_t).mean()
        loss = loss_t.mean()
        ...
        if (epoch + 1) % 100 == 0:
            ...
            }, './3Dnls/data driven/model/checkpoint_deeponet.pt')
...
def load_model(model):
    ...
    checkpoint = torch.load('./3Dnls/data driven/model/checkpoint_deeponet.pt')
...
def postprocess3D(data, x, y, z, flag=None, num=None, t=None):
    ...
    # fig.show()
    fig.write_image('./3Dnls/figures/deeponet/Iso_surf_deeponet_%s_%.2f.png' % (appd[flag], t[num]), engine='orca', scale=2)
    plt.close('all')
...
if __name__ == '__main__':
    ...
    # 访问各个数组
    uv_sol = data['u_sol']  # (3001,2,32,32,32)
    t = data['t']  # (3001,)
    ...
    U_abs = np.sqrt(output_u ** 2 + output_v ** 2)
...
#-----------PLOT---------------
    ...
    output_u = torch.cat(tuple(output_u), dim=1).T
    output_v = torch.cat(tuple(output_v), dim=1).T
    output_u = output_u.reshape(length, Nx, Ny, Nz).cpu().detach().numpy()
    output_v = output_v.reshape(length, Nx, Ny, Nz).cpu().detach().numpy()

    U_abs = np.sqrt(output_u ** 2 + output_v ** 2)
    truth_abs = np.sqrt(truth_u ** 2 + truth_v ** 2)
    for i in range(30, length, 5):
        postprocess3D(U_abs[i]-truth_abs[i], X, Y, Z, flag=2, num=i, t=t)