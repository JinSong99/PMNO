...
class RONN(nn.Module):
    ...
    def forward(self):
        ...
        for step in range(self.step - self.k):
            ...
            u = o[0, :, :, 0:1]
            Du = self.spectral_laplacian_2d(u.squeeze(-1))
            Du = Du.unsqueeze(-1)
            ...
            rloss = ((res * mask_3d.unsqueeze(0)) ** 2).mean().unsqueeze(0).unsqueeze(0)
    ...
    def spectral_laplacian_2d(self, u, Lx=1.0, Ly=1.0):
        """
        使用谱方法计算二维张量场 u 的拉普拉斯算子 ∇²u
        :param u: 输入张量 (Nx, Ny)
        :param Lx: x方向计算域长度
        :param Ly: y方向计算域长度
        :return: 拉普拉斯算子值 ∇²u，形状为 (Nx, Ny)
        """
        # 获取网格尺寸
        Nx, Ny = u.shape
        device = u.device
        ...
        # 生成频率坐标 (单位：弧度)
        kx = 2 * np.pi * torch.fft.fftfreq(Nx, d=Lx / Nx).to(device)  # (Nx,)
        ky = 2 * np.pi * torch.fft.fftfreq(Ny, d=Ly / Ny).to(device)  # (Ny,)

        # 构造频率网格
        Kx, Ky = torch.meshgrid(kx, ky, indexing='ij')  # indexing='ij' 匹配 x, y 维度

        # 傅里叶变换
        U = torch.fft.fft2(u)

        # 拉普拉斯在频域中是 - (Kx² + Ky²)
        laplacian_U = - (Kx ** 2 + Ky ** 2) * U

        # 逆变换回空间域，取实部
        laplacian_u = torch.fft.ifft2(laplacian_U).real
...
def train(model, n_iters, learning_rate, k, cont=True):
    ...
    for epoch in range(n_iters):
        ...
        optimizer.zero_grad()
        # One single batch
        loss_t, rloss_t = model()  # output is a list
        loss_t = torch.cat(tuple(loss_t), dim=1)  # N*(T-k)
        rloss_t = torch.cat(tuple(rloss_t), dim=1)  # N*(T-k)
        loss = loss_t.mean()
        ...
        if (epoch + 1) % 100 == 0:
            ...
            }, './2Dheat/physics driven/model/checkpoint_pifno.pt')
...
def load_model(model):
    ...
    checkpoint = torch.load('./2Dheat/physics driven/model/checkpoint_pifno.pt')
