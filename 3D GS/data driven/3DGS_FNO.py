...
def train(model, x, n_iters, learning_rate, cont=True):
    ...
    for epoch in range(n_iters):
        ...
        if (epoch + 1) % 100 == 0:
            ...
            }, './3D GS/data driven/model/checkpoint_fno1.pt')
...
def load_model(model):
    ...
    checkpoint = torch.load('./3D GS/data driven/model/checkpoint_fno1.pt')
...
def postprocess3D(data, x, y, z, isU=True, flag=None, num=None, t=None):
    ...
    # fig.show()
    fig.write_image('./3D GS/figures/FNO/Iso_surf_fno_%s_%s_%.2f.png' % (uv[isU], appd[flag], t[num]), engine='orca', scale=2)
    plt.close('all')
