...
def train(model, y, n_iters, learning_rate, cont=True):
    ...
    for epoch in range(n_iters):
        ...
        if (epoch + 1) % 100 == 0:
            ...
            }, './3D GS/physics driven/model/checkpoint_PI.pt')
...
def load_model(model):
    ...
    checkpoint = torch.load('./3D GS/physics driven/model/checkpoint_PI.pt')
...
def postprocess3D(data, x, y, z, isU=True, flag=None, num=None, t=None):
    ...
    # fig.show()
    fig.write_image('./3D GS/figures/PIdeeponet/Iso_surf_PIdeeponet_%s_%s_%.2f.png' % (uv[isU], appd[flag], t[num]), engine='orca', scale=2)
    plt.close('all')
...
if __name__ == '__main__':
    ...
    # equations
    # u_t = Du * Lu - 20 * u * v ** 2 + F * (1 - u)
    # v_t = Dv * Lv + 20 * u * v ** 2 - (F + k) * v
    ...
    Lu_e = np.mean(np.sqrt((output_u - truth_u) ** 2), axis=(1, 2, 3))
