...
class RONN(nn.Module):
    ...
    def __init__(self, modes, width, k, dt, init_h0, truth, x,
    ...
    def forward(self):
        ...
        for step in range(self.step - self.k):
            ...
            # hidden state + output
            o = getattr(self, name)(torch.cat((h, self.x), dim=-1))  # N*1
...
def train(model, n_iters, learning_rate, cont=True):
    ...
    for epoch in range(n_iters):
        ...
        if (epoch + 1) % 100 == 0:
            ...
            }, './2DRD/data driven/model/checkpoint_fno.pt')
...
def load_model(model):
    ...
    checkpoint = torch.load('./2DRD/data driven/model/checkpoint_fno.pt')
...
if __name__ == '__main__':
    ...
    def get_grid(k, Nx, Ny):
        batchsize, size_x, size_y = k, Nx, Ny
        gridx = torch.tensor(np.linspace(0, 10, size_x, endpoint=False), dtype=torch.float)
        gridx = gridx.reshape(1, size_x, 1, 1).repeat([batchsize, 1, size_y, 1])
        gridy = torch.tensor(np.linspace(0, 10, size_y, endpoint=False), dtype=torch.float)
        gridy = gridy.reshape(1, 1, size_y, 1).repeat([batchsize, size_x, 1, 1])
        return torch.cat((gridx, gridy), dim=-1).cuda()

    grid = get_grid(k, Nx, Ny)  # (k,128,128,2)
    ...
    # 创建 RONN 模型
    model = RONN(modes, width, k, dt, init_h0, truth, grid, step, effective_step).cuda()
    ...
    plt.figure(figsize=(8, 6))
    plt.imshow(np.abs(outputs_u[-1] - truth_u[-1]), aspect='auto', origin='lower',
               cmap='viridis')
    ...
    plt.figure(figsize=(8, 6))
    plt.imshow(truth_u[-1], aspect='auto', origin='lower',extent=[0, Lx, 0, Ly],
               cmap='viridis')
    plt.colorbar()
    # plt.title('Heatmap of u = exp(-(x - at)^2)')
    plt.xlabel('$x$', fontsize=14)
    ...
    plt.figure(figsize=(8, 6))
    plt.imshow(truth_v[-1], aspect='auto', origin='lower', extent=[0, Lx, 0, Ly],
               cmap='viridis')
    plt.colorbar()
    # plt.title('Heatmap of u = exp(-(x - at)^2)')
    plt.xlabel('$x$', fontsize=14)
