...
class RCNN(nn.Module):
    ...
    def forward(self, y, init_true):
        ...
        for step in range(self.step - self.k):
            ...
            Lu = self.compute_laplacian(u, y)
            Lv = self.compute_laplacian(v, y)
...
def train(model, y, init_true, n_iters, learning_rate, cont=True):
    if cont:
    ...
    for epoch in range(n_iters):
        ...
        optimizer.zero_grad()
        # One single batch
        rlossu_t, rlossv_t, loss_t = model(y, init_true)  # output is a list
        ...
        if (epoch + 1) % 100 == 0:
            ...
            }, './2DRD/physics driven/model/checkpoint_PI.pt')
...
def load_model(model):
    ...
    checkpoint = torch.load('./2DRD/physics driven/model/checkpoint_PI.pt')
...
if __name__ == '__main__':
    ...
    # equations
    # u_t = D*Lap u + (lam*u - omega*v) - beta * (u**2+v**2)*u
    # v_t = D*Lap v + (omega*u + lam*v) - beta * (u**2+v**2)*v
    ...
    train_loss_list, train_rloss_list = train(model, y, init_true, n_iters, learning_rate, cont=cont)
    end = time.time()
